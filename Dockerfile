...
ADD https://github.com/arut/nginx-rtmp-module/archive/v1.2.1.tar.gz ${SRC_BASE_DIR}
...
RUN yum -y install gcc openssl openssl-devel libxml2-devel libxslt-devel pcre-devel \
    ...
    && ./configure --prefix=/usr/local/nginx  --add-module=../nginx-rtmp-module-1.2.1  --with-http_ssl_module  --with-http_xslt_module \
    && make \
    && make install \
    && mv /usr/local/nginx-rtmp-module-1.2.1/stat.xsl /usr/local/nginx/conf/ 
